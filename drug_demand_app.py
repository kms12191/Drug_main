...
OUTPUT_DIR_CANDIDATES = [
    Path(os.getenv("DRUG_DEMAND_OUTPUT_DIR")) if os.getenv("DRUG_DEMAND_OUTPUT_DIR") else None,
    APP_DIR,
    APP_DIR / "outputs",
    APP_DIR / "data",
...
FORECAST_FILE_NAMES = [
    ...
    "baseline_best_model_2026_demand_forecast.csv",
    "random_forest_2026_demand_forecast.csv",
]
METRICS_FILE_NAMES = [
    "model_comparison_2025_metrics.csv",
    "baseline_models_2025_validation_metrics.csv",
    "random_forest_2025_validation_metrics.csv",
]
TEST_FILE_NAMES = [
    "model_comparison_2025_predictions.csv",
    "baseline_models_2025_test_predictions.csv",
    "random_forest_2025_test_predictions.csv",
]
...
def find_first_existing(file_names):
...
forecast_source = find_first_existing(FORECAST_FILE_NAMES)
metrics_source = find_first_existing(METRICS_FILE_NAMES)
test_source = find_first_existing(TEST_FILE_NAMES)

with st.sidebar:
    st.header("\ub370\uc774\ud130")
    uploaded_forecast_file = st.file_uploader("\uc608\uce21 \uacb0\uacfc CSV", type=["csv"])
    uploaded_metrics_file = st.file_uploader("2025\ub144 \uc131\ub2a5 CSV", type=["csv"])
    uploaded_test_file = st.file_uploader("2025\ub144 \uac80\uc99d CSV", type=["csv"])

if uploaded_forecast_file is not None:
    forecast = read_csv_with_fallback(uploaded_forecast_file)
elif forecast_source is not None:
    forecast = read_csv_path(forecast_source)
else:
    st.error("\uc608\uce21 \uacb0\uacfc CSV\uac00 \uc5c6\uc2b5\ub2c8\ub2e4. Hugging Face \uc800\uc7a5\uc18c\uc5d0 `model_forecast_2026_all_methods.csv`\ub97c \uc62c\ub9ac\uac70\ub098, \uc0ac\uc774\ub4dc\ubc14\uc5d0\uc11c \uc608\uce21 \uacb0\uacfc CSV\ub97c \uc5c5\ub85c\ub4dc\ud558\uc138\uc694.")
    st.stop()

metrics_data = None
if uploaded_metrics_file is not None:
    metrics_data = read_csv_with_fallback(uploaded_metrics_file)
elif metrics_source is not None:
    metrics_data = read_csv_path(metrics_source)

test_data = None
if uploaded_test_file is not None:
    test_data = read_csv_with_fallback(uploaded_test_file)
elif test_source is not None:
    test_data = read_csv_path(test_source)
